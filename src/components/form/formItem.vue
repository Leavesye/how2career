<template>
  ...
  >
    ...
    >
      <ax-template
        v-if="model.renderPrefix"
        :template="model.renderPrefix"
        slot="prefix"
      ></ax-template>
      <ax-template
        v-if="model.renderSuffix"
        :template="model.renderSuffix"
        slot="suffix"
      ></ax-template>
      <ax-template
        v-if="model.renderPrepend"
        :template="model.renderPrepend"
        slot="prepend"
      ></ax-template>
      <ax-template
        v-if="model.renderAppend"
    ...
    >
      <el-option
        v-for="o,i in (options || model.options)"
    ...
    -->
    <el-time-picker
      v-else-if="model.type == 'timePicker'"
      v-model="model.value"
      v-bind="model.props"
      v-on="model.events"
      class="form-item-input"
    ...
    >
      <el-radio-group
        v-if="model.props.radioButton == true"
        v-model="model.value"
        v-bind="model.props"
        v-on="model.events"
        :ref="model.type"
      >
        <el-radio-button
          ...
          :disabled="o.disabled"
          :key="i"
        >{{ o[(model.props.props && model.props.props.label) || 'label'] }}</el-radio-button>
      </el-radio-group>
      <el-radio-group
        v-else
        v-model="model.value"
      ...
      >
        <el-radio
          ...
          :disabled="o.disabled"
          :key="i"
        >{{ o[(model.props.props && model.props.props.label) || 'label'] }}</el-radio>
      </el-radio-group>
    </div>
    <!--
    radio 配置项参考
    ...
    >
      <el-checkbox-group
        v-if="model.props.checkboxButton == true"
        v-model="model.value"
        v-bind="model.props"
        v-on="model.events"
        :ref="model.type"
      >
        <el-checkbox-button
          ...
          :disabled="o.disabled"
          :key="i"
        >{{ o[(model.props.props && model.props.props.label) || 'label'] }}</el-checkbox-button>
      </el-checkbox-group>
      <el-checkbox-group
        v-else
        v-model="model.value"
      ...
      >
        <el-checkbox
          ...
          :disabled="o.disabled"
          :key="i"
        >{{ o[(model.props.props && model.props.props.label) || 'label'] }}</el-checkbox>
      </el-checkbox-group>
    </div>
    <el-checkbox
      v-if="model.type=='single-checkbox'"
      v-model="model.value"
      v-bind="model.props"
      v-on="model.events"
      :ref="model.type"
    >{{model.text}}</el-checkbox>
    <!--
    switch 配置项参考
    {
      label : 'a' ,
    ...
    -->
    <el-color-picker
      v-else-if="model.type == 'colorPicker'"
      v-model="model.value"
      v-bind="model.props"
      v-on="model.events"
      class="form-item-input"
    ...
    <div
      v-if="model.note"
      style="height: 24px; padding-top: 14px;color:#7c8ea5"
    >{{ model.note }}</div>
  </el-form-item>
</template>
...
export default {
  ...
  created() {
    ...
    if (this.model.props.placeholder == undefined) {
      let text =
        "input|inputNumber".indexOf(this.model.type) >= 0 ? "请输入" : "请选择";
